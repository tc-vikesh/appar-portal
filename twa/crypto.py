...
def encrypt_twa_payload(payload_dict):
    """
    Encrypts a dictionary payload into AES-256-GCM format.
    Returns a dictionary {"encryptedData": "<base64_encoded_string>"}
    """
    key_b64 = getattr(settings, 'TWA_ENCRYPTION_KEY', None) or getattr(settings, 'ABC_ENCRYPTION_KEY', None)
    if not key_b64:
        raise ValueError("TWA_ENCRYPTION_KEY is not set in settings.")
...
def decrypt_twa_payload(encrypted_b64):
    """
    Decrypts the TWA AES-256-GCM base64 encoded payload.
